...
def diff_rate(E_r, A):
    ...
    return constant * A * form_factor(E_r, A) ** 2 * vel_integral(M_T, E_r, mu)


def vel_integral(M_T, E_r, mu):
    v_min_arr = np.sqrt(M_T * E_r / (2 * mu ** 2))
    k = ((np.pi ** (3 / 2)) * (const.v_0 ** 3) * (
            sp.erf(const.v_esc / const.v_0) - 2 * const.v_esc * np.exp(-(const.v_esc / const.v_0) ** 2) / (
                math.sqrt(math.pi) * const.v_0))) ** (-1)
    g = 0
    integral_v = np.empty(v_min_arr.shape[0])
...
def form_factor(Er, A):
    ...
    qr = q * r / const.hc
    qs = q * const.skin / const.hc
    return 3. * (np.sin(qr) - qr * np.cos(qr)) / (qr ** 3.) * np.exp(-qs * qs / 2.)


def integrate_rate(Er, rate):
...
def maxRecoilEnergy(A):
    M_D = const.M_D
    M_n = const.Mn
    M_T = M_n * A
    mu = M_T * M_D / (M_T + M_D)
    return 2 * (const.v_esc + const.v_e) ** 2 * mu ** 2 / M_T/const.c**2


def integrate_rate_new(A):
    Emin = 0.001 * const.keV
    Emax = maxRecoilEnergy(A)
    Nsteps = 10000
    conv_fact = const.kg * const.day
